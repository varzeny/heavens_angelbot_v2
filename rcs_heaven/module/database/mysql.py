...
class Manager:
        ...
        async with self.pool.acquire() as conn:
            async with conn.cursor() as cur:
                if table != "unit_coffee":
                    ...
                    await cur.execute(
                        f"CREATE TABLE IF NOT EXISTS {table} (id INT AUTO_INCREMENT PRIMARY KEY, time DATETIME DEFAULT CURRENT_TIMESTAMP, flag_coffee BOOLEAN, flag_none BOOLEAN, f_status JSON )"
                    )
                await conn.commit()
            ...
            async with self.pool.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    if table != "unit_coffee":
                        ...
                        await cur.execute(
                            f"INSERT INTO {table} (flag_coffee, flag_none, f_status) VALUES (%s, %s, %s)",
                            (data[0], data[1], status_str)
                        )
    ...
    async def read_table(self, table, all=False):
        async with self.pool.acquire() as conn:
            async with conn.cursor() as cur:
                if all:
                    await cur.execute(
